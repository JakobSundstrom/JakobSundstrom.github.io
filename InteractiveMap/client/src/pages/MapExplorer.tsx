...
const MapExplorer: React.FC = () => {
  ...
  const createIcon = useCallback((category: 'mine' | 'smelter' | 'office', correct: boolean = false, incorrect: boolean = false) => {
  ...
  const updateTargetDisplay = (target: Location) => {
    ...
    if (targetNameRef.current) {
      ...
      let color = '#3B82F6'; // Default blue for smelter
      if (target.category === 'mine') {
        color = '#F59E0B'; // Yellow for mine
      } else if (target.category === 'office') {
  ...
  const endGame = useCallback(() => {
    ...
    // Sync all the ref values to state before calculating stats
    // This way we don't need state updates during gameplay
    ...
    // Now calculate stats using the synced values
    const totalLocations = locationData.length;
    const foundCount = foundLocationsRef.current.length;  // Use ref value directly
    const totalAttempts = foundCount + incorrectAttemptsValueRef.current;
    ...
    // Set game stats, using the ref values directly
    const stats = {
    ...
    // Save score to database if player exists
    if (currentPlayer) {
      submitScore({
  ...
  const startGame = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      if (targetNameRef.current) {
        ...
        // Set the target color
        let color = '#3B82F6'; // Default blue for smelter
        if (firstTarget.category === 'mine') {
          color = '#F59E0B'; // Yellow for mine
        } else if (firstTarget.category === 'office') {
          color = '#8B5CF6'; // Purple for office
        }
        targetColorRef.current = color;
        targetNameRef.current.style.color = color;
  ...
  const handleLocationClick = useCallback((location: Location) => {
    ...
    if (location.id === currentTargetRef.current.id) {
      ...
      // Don't update React state during gameplay to prevent flickering
      // We'll use the ref values when needed
      
      // Check if all locations found
      if (updatedFoundLocations.length >= locationData.length) {
        // End game directly - endGame will handle syncing state
        endGame();
      } else {
        // Set next target without re-rendering
      ...
      // Don't update React state during gameplay to prevent flickering
      // Only use refs for all game tracking
  ...
  // We use formatTime utility for display
  ...
  useEffect(() => {
    ...
    // Create map legend using control
    ...
    locationData.forEach(location => {
      ...
      // Only show tooltips in explore mode, not during gameplay
      if (gameState !== 'playing') {
        // Show the location and country but NOT the facility name
        marker.bindTooltip(`${location.location}, ${location.country}`, {
  ...
  return (
    <div className="h-screen w-full bg-gray-100 relative">
      <div className="h-full relative" ref={mapContainerRef}>
        {/* Map header with title and info button */}
        <div className="absolute top-4 left-4 p-4 z-[1000]">
          <div className="bg-white bg-opacity-70 backdrop-blur-sm py-2 px-4 rounded-xl shadow-lg flex items-center">
            <h1 className="text-2xl font-bold text-gray-800">Boliden Operations Map</h1>
            {gameState === 'idle' && (
              <button 
                className="bg-blue-600 p-2 rounded-full shadow-md hover:bg-blue-700 transition-all ml-3"
        ...
        {/* Simple Start Game button with transparency */}
        {gameState === 'idle' && !currentPlayer && (
          <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-[1000]">
            <PlayerForm onPlayerCreated={(player) => setCurrentPlayer(player)} />
          </div>
        )}
        ...
        {gameState === 'idle' && currentPlayer && (
          <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-[1000]">
            <button 
              className="px-10 py-5 bg-blue-600 bg-opacity-70 hover:bg-opacity-90 text-white text-2xl font-bold rounded-lg shadow-xl transition-all"
              onClick={startGame}
            >
              Start Game
            </button>
        ...
        {/* Info button is now integrated with the header */}
        
        {/* Info panel */}
        {gameState === 'idle' && showInfoPanel && (
          <div className="fixed top-20 left-4 z-[2000] w-80">
            <div className="bg-white bg-opacity-70 backdrop-blur-sm p-5 rounded-xl shadow-lg">
              <div className="flex justify-between items-center mb-3">
                <h2 className="text-xl font-bold">About Boliden Explorer</h2>
                <button 
              ...
              <p className="text-gray-600 mb-4">Test your knowledge of Boliden's mines, smelters, and offices across Europe.</p>
              ...
              <div className="text-xs text-gray-500">
                <div className="flex justify-between mb-1">
                  ...
                  <div>Offices: {locationData.filter(l => l.category === 'office').length}</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="fixed top-6 left-6 z-[1000]">
            <div className="bg-white bg-opacity-70 backdrop-blur-sm p-5 rounded-xl shadow-lg">
              <div className="text-center mb-4">
                <div className="text-sm text-gray-700 mb-1">Find this location:</div>
                <div className="text-2xl font-bold" ref={targetNameRef} style={{ color: targetColorRef.current }}>{currentTarget.title}</div>
              </div>
              
              <div className="flex justify-between gap-8 text-sm mb-3">
                <div className="flex flex-col items-center">
                  <span className="text-gray-700 mb-1">Score</span> 
                  <span className="text-lg font-bold" ref={scoreRef}>{score}</span>
                </div>
                <div className="flex flex-col items-center">
                  <span className="text-gray-700 mb-1">Found</span> 
                  <span className="text-lg font-bold" ref={foundCountRef}>{foundLocations.length}/{locationData.length}</span>
                </div>
                <div className="flex flex-col items-center">
                  <span className="text-gray-700 mb-1">Time</span> 
                  <span className="text-lg font-bold" ref={timerDisplayRef}>0:00</span>
                </div>
              </div>
              
              <div className="flex justify-center">
                <button 
                  className="py-2 px-6 bg-red-600 hover:bg-red-700 text-white rounded-md font-medium transition-colors"
                  onClick={endGame}
                >
                  Exit Game
                </button>
        ...
        {gameState === 'finished' && gameStats && (
          <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-[1000]">
            <div className="bg-white bg-opacity-70 backdrop-blur-sm p-8 rounded-xl shadow-lg w-[700px] max-w-[90vw]">
              <h2 className="text-2xl font-bold text-center mb-5">Game Results</h2>
              
              <div className="mb-5">
                <div className="text-center text-4xl font-bold text-blue-600 mb-2">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div>
                  <div className="mb-6 bg-white bg-opacity-60 rounded-lg p-3">
                    <div className="flex justify-between py-2 border-b border-gray-200">
                      <span className="text-gray-700">Total correct:</span>
                  ...
                  <div className="flex gap-4 mb-5 lg:mb-0">
                    ...
                    <button 
                      className="flex-1 py-3 px-4 bg-green-600 hover:bg-green-700 text-white rounded-md font-medium transition-colors"
                      onClick={startGame}
                    >
                      Play Again
                    </button>
                  ...
                  {!showLeaderboard && (
                    <button 
                      className="w-full mt-4 py-2 px-4 bg-gray-200 hover:bg-gray-300 text-gray-800 rounded-md font-medium transition-colors"
        ...
        {gameState !== 'playing' && selectedLocation && (
          <div className="absolute bottom-0 left-0 p-4 z-[1000] sm:max-w-md w-full">
            <div className="bg-white bg-opacity-70 backdrop-blur-sm p-5 rounded-xl shadow-lg">
              <div className="flex justify-between items-center mb-3">
                <h3 className="text-2xl font-bold">{selectedLocation.title}</h3>
                <button 
              ...
              <div className="text-base mb-4 flex items-center text-gray-700">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
                  <path fillRule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clipRule="evenodd" />
                </svg>
                <span className="font-medium">{selectedLocation.location}, {selectedLocation.country}</span>
              </div>
              
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div className="bg-white bg-opacity-60 p-3 rounded-lg">
                  <div className="text-sm uppercase font-semibold text-gray-600 mb-1">Type</div>
                  <div className="flex items-center">
                    <div className={`w-4 h-4 rounded-full mr-2 ${
                      selectedLocation.category === 'mine' 
                        ? 'bg-yellow-500' 
                        : selectedLocation.category === 'smelter'
                          ? 'bg-blue-500'
                          : 'bg-purple-500'
                    }`}></div>
                    <span className="capitalize font-medium">{selectedLocation.category}</span>
                  </div>
                </div>
                
                <div className="bg-white bg-opacity-60 p-3 rounded-lg">
                  <div className="text-sm uppercase font-semibold text-gray-600 mb-1">Operating Since</div>
                  <div className="font-medium">{selectedLocation.startYear}</div>
                </div>
              </div>
              
              <div className="mb-4 bg-white bg-opacity-60 p-3 rounded-lg">
                <div className="text-sm uppercase font-semibold text-gray-600 mb-2">Products</div>
                <div className="flex flex-wrap gap-2">
                  {selectedLocation.products.map((product, index) => (
                    <span 
                      key={index} 
                      className="inline-block bg-white px-3 py-1 rounded-md text-sm font-medium shadow-sm"
                    >
                      {product}
                    </span>
                  ))}
              ...
              <div className="bg-white bg-opacity-60 p-3 rounded-lg">
                <div className="text-sm uppercase font-semibold text-gray-600 mb-2">About</div>
                <p className="text-sm text-gray-700 leading-relaxed">{selectedLocation.details}</p>
              </div>
            </div>
          </div>
        )}
